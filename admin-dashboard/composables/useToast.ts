...
export function useToast() {
  ...
  const success = (summary: string, detail?: string, options?: Omit<ToastOptions, 'summary' | 'detail' | 'severity'>) => {
    show(summary, { ...options, summary, detail, severity: 'success' })
  }

  const error = (summary: string, detail?: string, options?: Omit<ToastOptions, 'summary' | 'detail' | 'severity'>) => {
    show(summary, { ...options, summary, detail, severity: 'error' })
  }

  const warn = (summary: string, detail?: string, options?: Omit<ToastOptions, 'summary' | 'detail' | 'severity'>) => {
    show(summary, { ...options, summary, detail, severity: 'warn' })
  }

  const info = (summary: string, detail?: string, options?: Omit<ToastOptions, 'summary' | 'detail' | 'severity'>) => {
